...
# TODO: Implement logging for base_patterns.py functions
...
# TODO: Delete safely, simpler version of truncated message generated by AI
...
def iter_chunked_payloads(
    ...
    parts = truncate_message(payload.content, max_length)
    if not parts:
...
def truncate_message(
    ...
    """Split Markdown-style messages into chunked content within ``max_length``.
    ...
    # Characters used to close-out an open 'fenced' code block
    chunk_fence_close = '\n```'

    chunks: list[str] = []
    remaining = content
    # Language specified in the chunked fenced content
    carry_lang: Optional[str] = None

    while remaining:
        prefix = f'```{carry_lang}\n' if carry_lang is not None else ''

        # Account for AI suffixes and chunk fences - room for actual characters
        headroom = max_length - reserve - len(prefix) - len(chunk_fence_close)
        if headroom < 1:
            headroom = max_length // 2

        # If remaining characters fit into chunk max_length we can finish
        if len(prefix) + len(remaining) <= max_length - reserve:
            chunks.append(prefix + remaining)
            break

        # Find clean breaking points that are close prefer \n
        ...
        # Check for inline code after splitting
        candidate = remaining[:split_at]
        backtick_count = candidate.count('`') - candidate.count('\\`')
        if backtick_count % 2 == 1:
            last_bt = candidate.rfind('`')
            # Find the last real backtick and go to a space or newline before it
        ...
        # Combine cleaned candid at chunk_body with remaining before split
        chunk_body = remaining[:split_at]
        remaining = remaining[split_at:].lstrip()
        full_chunk = prefix + chunk_body

        # Determine placement of fence openings/closings and set carry_lang
        # for next chunk
