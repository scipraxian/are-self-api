"""Build distribution script with remote agent control for the UE project.

This script combines LAN distribution with remote agent commands to kill
existing game instances, deploy new builds, restore user data, and relaunch
the game.
"""
...
ROBO_SILENT = ['/E', '/NJH', '/NJS', '/NFL', '/NDL', '/R:2', '/W:2']
...
def copy_with_progress(src, dst):
    ...
    for root, dirs, files in os.walk(src):
        ...
        for f in files:
            ...
            current_file_idx += 1
            print_progress(copied_bytes, total_size, prefix='   Deploying:')
    print()
...
def distribute_to_target(target):
    """Distributes build and controls agent for a single target.
    ...
    # Dynamic paths using Project Name from Config
    ...
    # 1. KILL
    logger.info('   [AGENT] Sending KILL to %s...', name)
    AgentUtils.send_agent_command(name, 'KILL')
    time.sleep(2)  # Give it a moment to release file locks

    # 2. BACKUP SAVES
    if os.path.exists(remote_game_saved):
        logger.info('   Backing up saves...')
        subprocess.run(
            ['robocopy', remote_game_saved, backup_dir] + ROBO_SILENT,
            shell=True,
            check=False
        )
    ...
    if os.path.exists(backup_dir):
        ...
        subprocess.run(
            ['robocopy', backup_dir, remote_game_saved] + ROBO_SILENT,
            shell=True,
            check=False
        )
    ...
    # 6. LAUNCH
    logger.info('   [AGENT] Sending LAUNCH to %s...', name)
    AgentUtils.send_agent_command(name, 'LAUNCH')


def main():
    """Main execution point for Step 6 with agent control."""
    print('=' * 49)
    print('   6. DISTRIBUTE + REMOTE AGENT (Config)')
    print('=' * 49)

    if not os.path.exists(SOURCE_DIR):
        logger.error('[ERROR] Source not found at %s', SOURCE_DIR)
        return

    for target in config.REMOTE_TARGETS:
        distribute_to_target(target)

    logger.info('\n[DONE] Distribution & Launch Cycle Complete.')
