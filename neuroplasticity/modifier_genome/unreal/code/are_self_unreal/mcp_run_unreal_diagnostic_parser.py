"""Parietal MCP tool: summarize errors/warnings in an Unreal Engine log.

Reads `Spike.application_log`, streams it through the UE run-log
strategy registered by the sibling `log_parsers` module, and returns
a paginated diagnostic summary suitable for an LLM tool result.
"""
...
from .log_parsers import LogConstants, LogParserFactory
...
def _parse_spike_log(spike_id: str, page: int = 1) -> str:
    try:
        ...
        error_indices = [
            ...
            if LogConstants.LVL_ERROR in e.level
            or getattr(e, 'message', '').find('Error') != -1
        ]
        warning_indices = [
            i
            for i, e in enumerate(entries)
            if LogConstants.LVL_WARNING in e.level
            or getattr(e, 'message', '').find('Warning') != -1
        ]
        ...
        err_line_str = (
            f" on lines {', '.join(err_lines[:5])}" if err_lines else ''
        )
        if len(err_lines) > 5:
            err_line_str += f'... (and {len(err_lines) - 5} more)'

        warn_line_str = (
            f" on lines {', '.join(warn_lines[:5])}" if warn_lines else ''
        )
        if len(warn_lines) > 5:
            warn_line_str += f'... (and {len(warn_lines) - 5} more)'

        summary_header = (
            f'Parser found {len(error_indices)} Critical Errors'
            f'{err_line_str}. Found {len(warning_indices)} Warnings'
            f'{warn_line_str}.\n'
        )
        ...
        errors_per_page = 5
        total_pages = (
            math.ceil(len(error_indices) / errors_per_page)
            if error_indices
            else 1
        )
        ...
        summary = summary_header + (
            f'\n[DIAGNOSTIC RESULTS: Page {page} of {total_pages} | '
...
async def mcp_run_unreal_diagnostic_parser(
    ...
    """Parse an Unreal Engine spike log and return a structural summary."""
