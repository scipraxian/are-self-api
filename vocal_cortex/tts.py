...
def _cached_audio_path(cache_dir: str, provider_name: str, voice: Optional[str], text: str) -> str:
    """Return path to cached audio file for this synthesis key."""
    name = _cache_key(provider_name, voice, text) + '.bin'
    return os.path.join(cache_dir, name)


def _load_tts_module(provider_name: str):
    """Import vocal_cortex.providers.tts_<provider_name>."""
    module_path = 'vocal_cortex.providers.tts_%s' % (provider_name,)
...
class TTSService(object):
    """Dispatches synthesis to configured TTS provider with optional file cache."""

    def __init__(self, vocal_settings: Optional[dict[str, Any]] = None):
        """If vocal_settings is None, use django.conf.settings.VOCAL_CORTEX."""
        self._settings = vocal_settings
    ...
    def _read_cache_if_present(
        ...
        if os.path.isfile(path):
            logger.info('[VocalCortex] TTS cache hit for provider %s.', provider_name)
    ...
    async def synthesize(
        ...
        if not self._settings.get('enabled', True):
            return failure_result('tts', 'TTS is disabled in settings.')

        provider_name = (self._settings.get('tts_provider') or '').strip()
        if not provider_name:
            return failure_result('tts', 'No tts_provider configured.')
        ...
        for attempt in range(attempts):
            try:
                ...
                logger.exception(
                    '[VocalCortex] TTS provider %s raised on attempt %s.',
                    provider_name,
                    attempt + 1,
                )
            ...
            if local:
                logger.warning(
                    '[VocalCortex] Local TTS provider %s failed: %s',
                    provider_name,
                    result.error,
                )
                return result
            ...
            if attempt + 1 < attempts:
                logger.info(
                    '[VocalCortex] Retrying TTS provider %s after %ss.',
                    provider_name,
    ...
    def _write_to_cache_if_configured(
        ...
        try:
            ...
            logger.warning('[VocalCortex] Could not write TTS cache: %s', exc)
