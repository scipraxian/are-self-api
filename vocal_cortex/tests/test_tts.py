"""Tests for vocal_cortex.tts.TTSService."""
...
from vocal_cortex.contracts import SynthesisResult, success_result
from vocal_cortex.tts import TTSService
...
class TTSServiceTests(SimpleTestCase):
    ...
    def test_synthesize_returns_failure_when_tts_disabled(self):
        """Assert disabled VOCAL_CORTEX yields typed failure."""
        svc = TTSService(
            {
                'enabled': False,
                'tts_provider': 'edge',
    ...
    @patch('vocal_cortex.tts._call_provider_synthesize', new_callable=AsyncMock)
    ...
    @patch('vocal_cortex.tts._call_provider_synthesize', new_callable=AsyncMock)
    @patch('vocal_cortex.tts.asyncio.sleep', new_callable=AsyncMock)
    ...
    def test_cache_hit_skips_provider_call(self, mock_call):
        ...
        from vocal_cortex.tts import _cached_audio_path
    ...
    @patch('vocal_cortex.tts._call_provider_synthesize', new_callable=AsyncMock)
    @patch('vocal_cortex.tts.shutil.copy2')
    def test_success_writes_cache_when_configured(self, mock_copy, mock_call):
        ...
        mock_call.return_value = success_result('edge', '/src/out.mp3', format='mp3')
    ...
    def test_uses_django_settings_when_no_explicit_config(self, mock_call):
        ...
        mock_call.return_value = success_result('edge', '/x.mp3')
        svc = TTSService()
        result = async_to_sync(svc.synthesize)('hello')
        self.assertTrue(result.success)
