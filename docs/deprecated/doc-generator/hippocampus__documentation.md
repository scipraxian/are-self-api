# Hippocampus Module — Comprehensive Documentation

## Summary

The **hippocampus** module is Talos's durable long-term memory store. It manages **Engrams**—permanent memory cards—using 768-dimensional vector embeddings (via `nomic-embed-text`), cosine-similarity screening to reject near-duplicates, and relational links to sessions, spikes, turns, identities, and tags. The module exposes an asynchronous `TalosHippocampus` engine and REST API for CRUD operations. It is invoked by the Thalamus (for catalog injection) and by Parietal MCP tools (`mcp_engram_save`, `mcp_engram_read`, `mcp_engram_update`, `mcp_engram_search`).
...
- **Thalamus** (`frontal_lobe.thalamus`): Injects the Engram catalog into the sensory state for each reasoning turn (Turn 1: Spike-linked catalog; Turn 2+: recent session engrams).
...
| `TalosEngram` | Model | Engram entity with vector, sessions, spikes, turns, tags, identity_discs |
...
1. **Catalog injection (Thalamus):**
   - On Turn 1: `relay_sensory_state` → `TalosHippocampus.get_turn_1_catalog(session.spike)` → formatted catalog string.
...
| Consumer | Usage |
|----------|-------|
| `frontal_lobe.thalamus.relay_sensory_state` | Injects catalog block into sensory state each turn |
...
flowchart TB
    ...
    relay -->|get_turn_1_catalog / get_recent_catalog| TH
    save --> TH
    read --> TH
    update --> TH
    search --> TH
