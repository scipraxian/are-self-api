...
The **central_nervous_system** (CNS) is Talos's graph-based orchestration engine. It executes labeled directed graphs of Neurons and Axons as waves of Spikes, driven by Celery tasks. The static graph is $G = (V, E, \lambda)$; the runtime is an unfolding execution tree with blackboard propagation, hierarchical subgraphs, and distribution modes.
...
- **peripheral_nervous_system**: `NerveTerminal` for local/remote execution
- **frontal_lobe**: Native handler `run_frontal_lobe` for reasoning spikes
- **temporal_lobe**: Native handler `run_temporal_lobe` for scheduler spikes
- **synaptic_cleft**: Websocket events (Glutamate, Dopamine, Cortisol, Acetylcholine)
...
| `NeuralPathway`, `Neuron`, `Axon`, `Effector`, `Spike`, `SpikeTrain` | Models | Graph and runtime entities |
...
| `synaptic_cleft` | `fire_neurotransmitter(Glutamate|Dopamine|Cortisol|Acetylcholine)` |
...
- **Distribution modes:** LOCAL_SERVER, ALL_ONLINE_AGENTS, ONE_AVAILABLE_AGENT, SPECIFIC_TARGETS
...
**Extension points:**
- Add native handlers in `NATIVE_HANDLERS` (generic_effector_caster.py)
...
|------|-------------|
| LOCAL_SERVER | 1 |
| ONE_AVAILABLE_AGENT | 1 if $n_{\text{online}} > 0$, else fail |
| ALL_ONLINE_AGENTS | $n_{\text{online}}$ |
| SPECIFIC_TARGETS | $n_{\text{targets}}$ |
