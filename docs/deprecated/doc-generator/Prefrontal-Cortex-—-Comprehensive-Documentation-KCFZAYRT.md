...
The **prefrontal\_cortex** module manages the Agile work graph (Epic → Story → Task) and acts as the "bouncer": it evaluates whether an `IterationShiftParticipant` has eligible work before creating a ReasoningSession. If no work exists, the worker is stood down.
