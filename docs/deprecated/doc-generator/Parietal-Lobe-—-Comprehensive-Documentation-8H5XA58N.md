...
The **parietal\_lobe** module is the tool bridge between the LLM and Talos. It builds machine-readable JSON schemas from database metadata, executes tool calls via `ParietalMCP`, and applies the focus/XP economy (cost, reward, fizzle). Tool calls are sorted by `focus_modifier` descending.
