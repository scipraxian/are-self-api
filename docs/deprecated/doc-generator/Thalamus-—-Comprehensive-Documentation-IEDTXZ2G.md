...
The **thalamus** module exposes the chat-bubble REST API for the dedicated `NeuralPathway.THALAMUS` pathway. It manages standing spike trains, injects human replies into paused reasoning sessions, and broadcasts entity state changes to **synaptic\_cleft** (Dopamine, Cortisol, Acetylcholine) across many model types. The ORM module `thalamus.models` only defines a lightweight `Stimulus` helper; persistence lives in CNS and frontal\_lobe.
