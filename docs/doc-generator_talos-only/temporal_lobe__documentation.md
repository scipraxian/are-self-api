...
The **temporal_lobe** module manages time-sliced work: Iterations, Shifts, and IterationShiftParticipants. It advances shifts when all participants are COMPLETED, cleans up ghost workers, and auto-incepts the next iteration loop (Ouroboros Protocol).
