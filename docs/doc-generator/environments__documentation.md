...
The **environments** module defines the global project context: `ProjectEnvironment`, executable templates, context variables, and environment selection. Exactly one environment can be selected at a time. Variable interpolation uses Django's template engine via `VariableRenderer`.
...
- **Effector.get_full_command**: Uses VariableRenderer with context
- **temporal_lobe**: Iteration links to environment
...
- **KEY_SERVER:** socket.gethostname() in base context
...
### Context Precedence (in resolve_environment_context)
