...
The **synaptic\_cleft** module provides real-time push of execution state to the UI via WebSocket neurotransmitter events. Biologically themed: Glutamate (log), Dopamine (positive status), Cortisol (negative status), Acetylcholine (memory/blackboard).
