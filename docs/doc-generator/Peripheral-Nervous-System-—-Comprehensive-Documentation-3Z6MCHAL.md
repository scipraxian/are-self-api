...
The **peripheral\_nervous\_system** module discovers remote agents via subnet scan and executes commands locally or remotely. `NerveTerminal` provides a unified async pipeline yielding `NerveTerminalEvent` (T\_LOG, T\_EXIT) for both paths.
