...
class StagingOpsIntegrationTest(TestCase):
    ...
    def setUp(self):
        ...
        self.exe_uat = HydraExecutable.objects.create(
            name='test UAT', slug='test_uat', path_template='RunUAT.bat'
        )
        self.exe_game = HydraExecutable.objects.create(
            name='tes Game', slug='test_game', path_template='Game.exe'
        )

        self.spell_build = HydraSpell.objects.create(
            name='TestStaging: Build Game', executable=self.exe_uat, order=10
        )
        self.spell_run = HydraSpell.objects.create(
            name='TestStaging: Record PSOs', executable=self.exe_game, order=20
        )
