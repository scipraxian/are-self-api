...
class GraphWalkerSubGraphTest(TestCase):
    def setUp(self):
        ...
        self.env = ProjectEnvironment.objects.create(name="Test Env", type=env_type, status=env_status)
        ...
        self.spawn_status = HydraSpawnStatus.objects.get_or_create(id=1, defaults={'name': 'Created'})[0]
        self.head_status = HydraHeadStatus.objects.get_or_create(id=1, defaults={'name': 'Created'})[0]
    ...
    def test_subgraph_inherits_environment(self):
        ...
        # Assert inheritance
        self.assertIsNotNone(child_spawn.environment, "Child spawn lost the environment context!")
        self.assertEqual(child_spawn.environment.id, self.env.id, "Child spawn inherited the WRONG environment!")