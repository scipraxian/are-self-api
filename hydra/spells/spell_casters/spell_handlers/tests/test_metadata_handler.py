...
MODULE_PATH = 'hydra.spells.spell_casters.spell_handlers.version_metadata_handler'
...
class VersionMetadataHandlerTest(TestCase):
    ...
    def setUp(self):
        self.status_running = HydraHeadStatus.objects.get(id=HydraHeadStatus.RUNNING)
        self.spawn_running = HydraSpawnStatus.objects.get(id=HydraSpawnStatus.RUNNING)

        self.spell = HydraSpell.objects.create(name="Version Spell",
    ...
    def test_creates_new_file_if_missing(self, mock_log, mock_resolve):
        ...
        with patch('os.path.exists') as mock_exists, \
            ...
            def exists_side_effect(path):
                if path == os.path.dirname(target_path): return True
    ...
    def test_updates_existing_file_preserving_data(self, mock_log, mock_resolve):
        ...
        with (patch('os.path.exists', return_value=True),
              patch('builtins.open', new_callable=mock_open,
                    read_data=existing_content) as mock_file):
            code, log = update_version_metadata(self.head.id)
            ...
            # Assertions
            self.assertEqual(data["Game"]["Major"], 9, "Should preserve existing Game data")
    ...
    def test_recovers_from_corrupt_json(self, mock_log, mock_resolve):
        ...
        with (patch('os.path.exists', return_value=True),
              patch('builtins.open', new_callable=mock_open, read_data="{ bad_json: ") as mock_file):
            code, log = update_version_metadata(self.head.id)
