...
class ReasoningEngine:
    ...
    def tick(self, session_id: int) -> None:
        ...
        # 7. Commit Turn
        self._commit_turn(
            session, response_text, context['user'], tool_result, action
        )
    ...
    def _handle_goal_preemption(self, session: ReasoningSession) -> None:
        ...
        """
        new_goal = (
            session.goals.filter(status_id=ReasoningStatusID.PENDING)
            .order_by('created')
            .first()
        )
        active_goal = (
            session.goals.filter(status_id=ReasoningStatusID.ACTIVE)
            .order_by('created')
            .first()
        )
    ...
    def _build_context(self, session: ReasoningSession) -> Dict[str, str]:
        ...
        if history_turns:
            ...
            for t in history_turns:
                ...
                goal_context = (
                    f' (Goal: {t.active_goal.reasoning_prompt})'
                    if t.active_goal
                    else ''
                )
                ...
                for call in t.tool_calls.all():
                    res_snippet = (
                        (call.result_payload[:800] + '...')
                        if len(call.result_payload) > 800
                        else call.result_payload
                    )
                    history_text += (
                        f'SYSTEM (Result {call.tool.name}): {res_snippet}\n'
                    )
    ...
    def _query_brain(self, context: Dict[str, str]) -> Optional[str]:
        ...
        try:
            result = client.chat(
                context['system'], context['user'], options={'temperature': 0.1}
            )
    ...
    def _execute_tool(
        self, action: Dict[str, Any], session: ReasoningSession
    ) -> str:
    ...
    def _update_rolling_summary(
        self, session: ReasoningSession, goal: ReasoningGoal
    ) -> None:
        ...
        try:
            res = client.chat(
                'SYSTEM: You are a technical summarizer.', summary_prompt
            )
