...
def _transcribe_sync(
    ...
    if WhisperModel is None:
        logger.error(
            '[VocalAuditoryCortex] faster_whisper package is not installed.'
        )
        return stt_failure_result(
            provider_name,
    ...
    try:
        ...
        logger.exception('[VocalAuditoryCortex] faster_whisper transcribe failed.')
        return stt_failure_result(
            provider_name, 'faster_whisper error: %s' % (exc,)
        )
