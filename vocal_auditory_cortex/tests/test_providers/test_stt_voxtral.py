...
from vocal_auditory_cortex.contracts import stt_success_result
from vocal_auditory_cortex.providers import stt_voxtral
...
class SttVoxtralTests(SimpleTestCase):
    ...
    @patch(
        'vocal_auditory_cortex.providers.stt_voxtral.voxtral_runtime.transcribe_with_voxtral',
    )
    def test_transcribe_delegates_to_runtime(self, mock_tx):
        """Assert provider calls voxtral_runtime.transcribe_with_voxtral."""
        mock_tx.return_value = stt_success_result('voxtral', 'hello', language='en')
