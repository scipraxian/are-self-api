"""Tests for auditory_cortex.stt.STTService."""
...
from auditory_cortex.contracts import TranscriptionResult, success_result
from auditory_cortex.stt import STTService
...
class STTServiceTests(SimpleTestCase):
    ...
    def test_transcribe_returns_failure_when_stt_disabled(self):
        """Assert disabled AUDITORY_CORTEX yields typed failure."""
        svc = STTService(
            {
                'enabled': False,
    ...
    @patch('auditory_cortex.stt._call_provider_transcribe', new_callable=AsyncMock)
    def test_transcribe_dispatches_to_provider(self, mock_call):
        ...
        mock_call.return_value = success_result(
            'faster_whisper',
            'hello',
            language='en',
        )
    ...
    @patch('auditory_cortex.stt._call_provider_transcribe', new_callable=AsyncMock)
    @patch('auditory_cortex.stt.asyncio.sleep', new_callable=AsyncMock)
    def test_remote_provider_retries_once_on_failure(self, mock_sleep, mock_call):
        ...
        ok = success_result('voxtral', 'fixed')
    ...
    def test_provider_exception_becomes_typed_failure(self, mock_call):
        """Assert provider exceptions map to TranscriptionResult with error set."""
    ...
    def test_uses_django_settings_when_no_explicit_config(self, mock_call):
        """Assert STTService reads AUDITORY_CORTEX from Django settings."""
        mock_call.return_value = success_result('faster_whisper', 'ok')
        svc = STTService()
        result = async_to_sync(svc.transcribe)('/tmp/x.wav')
        self.assertTrue(result.success)
