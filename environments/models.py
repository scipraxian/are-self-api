...
class ProjectEnvironment(models.Model):  # depreciated.
    ...
    name = models.CharField(
        max_length=100, unique=True, help_text='e.g., Mike Desktop 2024'
    )

    # Paths
    project_root = models.CharField(
        max_length=500, help_text='Path to .uproject folder', blank=False, null=False
    )
    engine_root = models.CharField(
        max_length=500, help_text='Path to UE_5.6 folder', blank=False, null=False
    )
    build_root = models.CharField(
        max_length=500, help_text='Path to BUILD output', blank=False, null=False
    )
    project_name = models.CharField(
        max_length=100,
        help_text='The EXE Prefix (e.g., HSHVacancy)',
        blank=False,
        null=False,
    )
    staging_dir = models.CharField(
        max_length=500, blank=True, null=True, help_text='Path to Staging output'
    )

    agent_port = models.IntegerField(
        default=5005, help_text='Port the agent is listening on'
    )
    is_active = models.BooleanField(
        default=False, help_text='Only one build environment can be active at a time.'
    )
...
class TalosExecutable(DefaultFieldsMixin, DescriptionMixin):
    ...
    UNREAL_AUTOMATION_TOOL = (
        5  # C:\\Program Files\\Epic Games\\UE_5.6/Engine/Build/BatchFiles/RunUAT.bat
    )
    ...
    executable = models.CharField(
        max_length=500, help_text='Full path to the executable, including filename.'
    )
