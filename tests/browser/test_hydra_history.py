...
from hydra.models import HydraSpellbook, HydraSpawn, HydraSpawnStatus, HydraHeadStatus, HydraEnvironment, HydraSpell, HydraExecutable, HydraHead
...
class HydraHistoryTests(StaticLiveServerTestCase):
    ...
    def test_history_visibility_and_navigation(self):
        with sync_playwright() as p:
            ...
            expect(page.get_by_text("No mission history available.")).to_be_visible()
            ...
            HydraHead.objects.create(spawn=spawn, spell=spell, status_id=HydraHeadStatus.SUCCESS)
            ...
            expect(page.locator(".history-item").first).to_contain_text("Fast Validate")
            expect(page.locator(".history-item").first).to_contain_text("Success")
            ...
            HydraHead.objects.create(spawn=spawn2, spell=spell, status_id=HydraHeadStatus.RUNNING)
            ...
            expect(page.locator(".history-item").first).to_contain_text("Running")
            expect(page.locator(".history-item").nth(1)).to_contain_text("Success")
            
            # 6. Click history item to navigate
            page.locator(".history-item").nth(1).click()
            
            # 7. Verify we are on the monitor page (full page)
            expect(page).to_have_url(f"{self.live_server_url}/hydra/monitor/{spawn.id}/?full=True")
            expect(page.get_by_text(f"OPERATION: {self.book.name}")).to_be_visible()
