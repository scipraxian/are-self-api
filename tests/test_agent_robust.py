...
import os
import unittest
from talos_agent.bin.agent_service import TalosAgent, VERSION
...
class TestAgentRobust(unittest.TestCase):
    ...
    def test_launch_parameters(self):
        res = self._send({'cmd': 'LAUNCH', 'args': {'exe_path': 'non_existent.exe', 'params': []}})
        self.assertEqual(res.get('status'), 'ERROR')

    def test_graceful_kill_logic(self):
        res = self._send({'cmd': 'KILL', 'args': {'process_name': 'non_existent_proc'}})
    ...
    def test_large_payload(self):
        ...
        self.assertEqual(res.get('status'), 'OK') # Probe still works, just returns exists=False

    def test_missing_args(self):
        '''Verify agent handles commands with missing arguments dict.'''
        # Missing 'args' completely
        res = self._send({'cmd': 'STATUS'})
        self.assertEqual(res.get('status'), 'OK') # Status defaults to empty metrics
