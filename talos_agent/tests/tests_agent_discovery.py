...
class AgentDiscoveryTests(TestCase):
    ...
    def test_discovery_task(self):
        ...
        try:
            ...
            self.assertIn(
                'ReleaseTest/TalosTest.exe',
                self.target.remote_exe_path.replace('\\', '/'),
            )
        finally:
            # Cleanup
            import shutil
            ...
            shutil.rmtree(
                os.path.join(os.getcwd(), 'ReleaseTest'), ignore_errors=True
            )
    ...
    def test_status_command_with_log_heartbeat(self):
        ...
        res = client.get_status(
            process_name='python', log_path=os.path.abspath(log_file)
        )
