"""Agile shift-assignment handler.

Resolves the PFC ticket locked to this disc and injects hyper-focused shift
instructions as a volatile system message. Ported from
identity/addons/agile_addon.py.
"""
import json
...
def _pm_instructions_sifting(shift_id, item_type, item_id):
    return f"""\
...
def _pm_instructions_pre_planning(shift_id, item_type, item_id):
    return f"""\
...
def _pm_instructions_post_execution(shift_id, item_type, item_id):
    return f"""\
...
def _build_pm_instructions(shift_id, item_type, item_id):
    builder = _PM_INSTRUCTION_BUILDERS.get(shift_id)
    if builder:
        return builder(shift_id, item_type, item_id)
    return (
        'You have no actionable tickets for this shift. '
        'Sleep and consolidate your memories.'
    )


def _worker_instructions_sifting(item_type, item_id):
    return f"""\
...
def _worker_instructions_pre_planning(item_type, item_id):
    return f"""\
...
def _worker_instructions_executing(item_type, item_id):
    return f"""\
...
def _worker_instructions_post_execution(item_type, item_id):
    return f"""\
...
def _build_worker_instructions(shift_id, item_type, item_id):
...
def _build_assignment_content(shift_id, item_type, item_id, item_json, is_pm):
...
class Agile(IdentityAddonHandler):
