"""Telemetry handler — system diagnostics, latency, cognitive-load warnings."""
import json
...
class Telemetry(IdentityAddonHandler):
    def on_context(self, turn: ReasoningTurn) -> List[Dict[str, Any]]:
        ...
        if l1_cache_size > 20000:
            pressure_warning = (
                ...
                f'Your recent memory buffer is massive '
                f'({l1_cache_size} characters). '
                'If you pull more data, you will suffer a fatal crash. '
                'You MUST use `mcp_pass` to rest and flush old data out of '
                'your L1 cache before proceeding.\n'
            )
        elif l1_cache_size > 12000:
            pressure_warning = (
                '\n[WARNING: CONTEXT PRESSURE RISING]\n'
                f'Your memory buffer is getting heavy '
                f'({l1_cache_size} characters). '
                'Consider saving your findings to Engrams and using '
                '`mcp_pass` to clear your cache.\n'
            )

        diagnostics = (
            f'[SYSTEM DIAGNOSTICS]\n'
            f'[CYCLE {current_turn} / {max_turns}] | '
            f'Speedrun Bounty: {remaining_turns * 1000} XP\n'
            f'Output Footprint (Prev Turn): {last_output_len} / '
            f'{target_capacity} chars -> Efficiency Bonus: '
            f'{efficiency_status}\n'
